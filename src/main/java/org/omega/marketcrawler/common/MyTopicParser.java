						...
						if (cr.size() > 6) {
							...
							if (Utils.isNotEmpty(lastPostTxt)) {
								...
								if (ss.length > 0 && Utils.isNotEmpty(ss[0])) {
									...
									lastPost = Utils.parsePostDate(lastPostTxt);
								}
							}
							
							if (lastPost != null) bean.setLastPostTime(Utils.changeMillsToSeconds(lastPost.getTime()));
						}
						
						cos.add(bean);
					}
				} catch (Exception e) {
					log.error("extract topic error.", e);
				}
			}
		}
